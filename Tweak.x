...
#define LoopStatusKey @"YouLoopLoopStatus"
...
- (void)didPressYouLoop {
    ...
    if ([mainAppController isKindOfClass:objc_getClass("YTMainAppVideoPlayerOverlayViewController")]) {
        ...
        // Display snackbar
        [[%c(GOOHUDManagerInternal) sharedInstance] showMessageMainThread:[%c(YTHUDMessage) messageWithText:(isLoopEnabled ? LOC(@"LOOP_ENABLED") : LOC(@"LOOP_DISABLED"))]];
    }
}
%end
